 ...
 */
export class SwitchToWindowByIndexActionProperties implements IActionProperties {
  /**
...
export const SwitchToWindowByIndexActionTypeAliases = ['switch-by-index'] as const;
...
export class SwitchToWindowByIndexAction extends IAction<SwitchToWindowByIndexActionProperties> {
  ...
  constructor(
    ...
    super(props);
    this.logger = loggerFactory.get<SwitchToWindowByIndexAction>(SwitchToWindowByIndexAction);
  }

  public async run(state: IState): Promise<void> {
    const allHandles = await state.currentDriver.getAllWindowHandles();
    const index = this.props.index >= 0? this.props.index : allHandles.length + this.props.index; // using + sign as index will already contain minus sign

    const handle = allHandles[index];
    if (!handle) {
      throw new InvalidOperationException(`Can't get a handle for the window with index '${this.props.index}'`);
    }
